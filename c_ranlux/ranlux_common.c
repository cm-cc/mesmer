...
*   myint8_t (*state)[4];          Current state vectors and carry bits.
*
...
* reserved for the carry bits. The data type myint8_t is defined in ranlux.h
...
static const myint8_t base=(myint8_t)(0x1000000000000);
static const myint8_t mask=(myint8_t)(0xffffffffffff);


int rlx_check_machine(void)
{
#if ((defined AVX2)||(defined SSE2))
   if ((sizeof(int*)>sizeof(unsigned long))||(sizeof(myint8_t)!=8))
      return 1;
#endif
...
void rlx_alloc_state(rlx_state_t *s)
{
   ...
   (*s).state=(myint8_t(*)[4])(((unsigned long)(addr+32))&(~mask));
#else
...
void rlx_init(rlx_state_t *s,int seed,int flag)
{
   ...
   for (i=0;i<4;i++)
   {
      for (k=0;k<24;k++)
      {
         ...
         if (k&0x1)
            state[k/2][i]+=((myint8_t)(ix)<<24);
         else
            state[k/2][i]=(myint8_t)(ix);
      }
   }

   (*s).state[12][0]=(myint8_t)(0);
   (*s).state[12][1]=(myint8_t)(0);
   (*s).state[12][2]=(myint8_t)(0);
   (*s).state[12][3]=(myint8_t)(0);
   (*s).ir=0;
}
...
{
   int pr,ir,k;
   myint8_t shift,(*pmin)[4],(*pmax)[4],(*pi)[4],(*pj)[4];
...
{
   int pr,ir,k;
   myint8_t d[4],(*pmin)[4],(*pmax)[4],(*pi)[4],(*pj)[4];
...
{
   int k;
   myint8_t lmask,shift,(*state)[4];
   float onebit;

   onebit=(float)(ldexp(1.0,-24));
   lmask=(myint8_t)(0xffffff);
   shift=(myint8_t)(24);
   state=(*s).state;
...
{
   int k;
   myint8_t sexp,(*state)[4];

   sexp=(myint8_t)(0x4030000000000000);
   state=(*s).state;
...
void rlx_set_state(int *is,rlx_state_t *s)
{
   ...
   for (k=0;k<12;k++)
   {
      (*state)[0]=(myint8_t)(is[0]);
      (*state)[1]=(myint8_t)(is[1]);
      (*state)[2]=(myint8_t)(is[2]);
      (*state)[3]=(myint8_t)(is[3]);

      (*state)[0]+=((myint8_t)(is[4])<<24);
      (*state)[1]+=((myint8_t)(is[5])<<24);
      (*state)[2]+=((myint8_t)(is[6])<<24);
      (*state)[3]+=((myint8_t)(is[7])<<24);
